...
/*  MOD (Mode) Field Encooding
    00 - Memory Mode , no displacement follows*
    01 Memory Mode, 8-bit displacemente follows
    10 Memory Mode, 16-bit displacement follows
    11 Register Mode (no displacement)

    * Except when R/M = 110 (Direct Address),  then 16-bit displacement follows
    ...
    110 |   DH  |  SI   | 110 | DIRECT ADDRRESS | (BP) + D8        | (BP) + D16
    111 |   BH  |  DI   | 111 | (BX)            | (BX) + D8        | (BX) + D16

    Table 4-12. 8086 Instruction Encoding pg. 164
*/
...
void disassemble_mov(const uint8_t *code, size_t *i, size_t length) {
    ...
    if (opcode >= 0xB0 && opcode <= 0xBF) {
        ...
        if (*i >= length) { printf("; Unexpected EOF\n"); return; }
        ...
        if (w == 0) {
            ...
            if (*i + 1 >= length) { printf("; Unexpected EOF\n"); return; }
    ...
    if (opcode >= 0x88 && opcode <= 0x8B) {
        ...
        int16_t disp = 0;
        if (mod == 0b01) { // 8-bit signed displacement
            if (*i >= length) { printf("; Unexpected EOF\n"); return; }
            disp = (int8_t)code[*i];
            (*i)++;
